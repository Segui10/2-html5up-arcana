...
function upload_files() {
    ...
    ////////////////////////////////////////////////////////////////////////////
    $upfile = $_SERVER['DOCUMENT_ROOT'].'/1_php_mvc_oo/1_Backend/0_template/1 templates_web_examples/2 html5up-arcana/media/'.$_FILES['avatar']['name'];
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
            ...
            $upfile = $_SERVER['DOCUMENT_ROOT'].'/1_php_mvc_oo/1_Backend/0_template/1 templates_web_examples/2 html5up-arcana/media/'.$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    }
    ...
    if ($error == "") {
        ...
        if($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
            $upfile ='/1_php_mvc_oo1_Backend/0_template/1 templates_web_examples/2 html5up-arcana/media/default-avatar.png';
            return $return=array('resultado'=>true,'error'=>$error,'datos'=>$upfile);
        }
    }else{
        return $return=array('resultado'=>false,'error'=>$error,'datos'=>"");
...
function remove_files(){
	$name = $_POST["filename"];
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/1_php_mvc_oo/1_Backend/0_template/1 templates_web_examples/2 html5up-arcana/media/'.$name)){
		unlink($_SERVER['DOCUMENT_ROOT'].'/1_php_mvc_oo/1_Backend/0_template/1 templates_web_examples/2 html5up-arcana/media/'.$name);
		return true;
	}else{
		return false;
	}
